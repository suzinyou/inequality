...
proc sql;
    ...
    select
        ...
        min(&col) as min,
        std(&col) as std
    from store.bfc_&reg group by STD_YYYY;
    quit;
run;
...
proc sql;
    ...
    select
        ...
        min(&col) as min,
        std(&col) as std
    from store.bfc_&reg group by STD_YYYY, sido;
    quit;
run;
...
proc sql;
    ...
    select
		...
		, min(&col) as min
		, std(&col) as std
    from store.bfc_&reg group by STD_YYYY, sido, sigungu;
    quit;
run;
...
proc export data=out.bfc_seoul_desc_stat_sidosigungu
	outfile="/userdata07/room285/data_out/data_out/bfc_seoul_desc_stat_sidosigungu.csv"
	replace;
run;

proc export data=out.bfc_smpl_desc_stat_sido
	outfile="/userdata07/room285/data_out/data_out/bfc_smpl_desc_stat_sido.csv"
	replace;
run;
...
proc export data=out.bfc_smpl_desc_stat_yearonly
	outfile="/userdata07/room285/data_out/data_out/bfc_smpl_desc_stat_yearonly.csv"
	replace;
run;
